...
{
	i2c_bus = (struct rt_i2c_bus_device *)rt_device_find("sci9i");
    if (i2c_bus == RT_NULL)
    {
        rt_kprintf("can't find sci9i device!\n");
    }
	else
	{
		rt_kprintf("sci9i device open success\n");
	}

}
INIT_DEVICE_EXPORT(mlx90640_init);

int _mlx90640_i2cread(struct rt_i2c_bus_device *bus, uint8_t slave_addr, uint16_t start_addr, uint16_t num, uint16_t *data)
{
    uint8_t *bp;
    rt_ssize_t ret;
    uint8_t start[2];
...
//	rt_kprintf("data_check=0x%x    data=0x%x\n",data_check,data);
    if ( data_check != data)
    {
        return -2;
